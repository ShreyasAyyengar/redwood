...
const wsPath = env.EXPO_PUBLIC_NODE_ENV === "development" ? "/api/ws" : "/ws";
const wsProtocol = env.EXPO_PUBLIC_API_URL.startsWith("https") ? "wss" : "ws";

const wsUrl = `${wsProtocol}://${new URL(env.EXPO_PUBLIC_API_URL).host}${wsPath}` as const;
