...
export function GroupClassroomSelector() {
  ...
  const { classrooms, selectedClassroomIds, selectAllClassrooms, deselectAllClassrooms } = useGroupStore();
  ...
  const filteredClassrooms = classrooms.filter(
    (c) =>
      ...
      c.groupKey.toLowerCase().includes(searchQuery.toLowerCase()) ||
      c.sourceRoomName.toLowerCase().includes(searchQuery.toLowerCase())
  );
    ...
    <div className="flex h-full flex-col rounded-lg border border-zinc-800 bg-neutral-900 p-5">
      <div className="mb-4 flex shrink-0 flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-xs text-zinc-400">Select classrooms to batch edit their groups</p>
        </div>
          ...
          <ScrollArea className="h-full px-3">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {filteredClassrooms.map((classroom) => (
                <GroupClassroomCard key={classroom._id} classroom={classroom} />
              ))}
            </div>
          </ScrollArea>
        </div>
