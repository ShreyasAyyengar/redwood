...
export default function Home() {
  ...
  const { data: countFromQuery } = useQuery(webClientORPC.counter.getCounter.queryOptions());
  ...
  const incrementMutation = useMutation(
    webClientORPC.counter.incrementCounter.mutationOptions({
      onSuccess: () =>
        queryClient.invalidateQueries({
          queryKey: webClientORPC.counter.getCounter.queryKey(),
        }),
    })
  );

  const decrementMutation = useMutation(
    webClientORPC.counter.decrementCounter.mutationOptions({
      onSuccess: () =>
        queryClient.invalidateQueries({
          queryKey: webClientORPC.counter.getCounter.queryKey(),
        }),
  ...
  const resetMutation = useMutation(
    webClientORPC.counter.resetCounter.mutationOptions({
      onSuccess: () =>
        queryClient.invalidateQueries({
          queryKey: webClientORPC.counter.getCounter.queryKey(),
        }),
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center gap-8 bg-black p-4 font-sans text-white">
        ...
        <div ref={scrollRef} className="scrollbar-thin scrollbar-thumb-zinc-800 flex flex-1 flex-col gap-2 overflow-y-auto p-2">
          <AnimatePresence initial={false}>
            {messages.map((msg, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, x: msg.from === "web" ? 20 : -20 }}
                animate={{ opacity: 1, x: 0 }}
                className={`flex flex-col ${msg.from === "web" ? "items-end" : "items-start"}`}
              >
                <span className="mb-1 px-1 text-[10px] text-zinc-500">{msg.from}</span>
                <div
                  className={`max-w-[80%] rounded-2xl px-4 py-2 text-sm ${
                    msg.from === "web" ? "rounded-tr-none bg-blue-600 text-white" : "rounded-tl-none bg-zinc-800 text-zinc-200"
                  }`}
                >
                  {msg.content}
                </div>
      ...
      >
        Interactive Modern Counter & Chat Built with Next.js & Framer Motion
      </motion.div>
    </div>
  );
}
