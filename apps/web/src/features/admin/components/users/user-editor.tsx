...
import { roles } from "@backend/convex/core/users/schemas";
...
export default function UserEditor() {
  const fetchedUsers = useQuery(api.core.users.service.getUsers, {});
  const users = fetchedUsers ?? [];
  const isLoading = fetchedUsers === undefined;
  ...
  const handleAddUser = async () => {
    ...
    try {
      await addUser({ email: `${newCruzid.trim()}@ucsc.edu`, role: newRole });
      setNewCruzid("");
    } finally {
      ...
      <div className="mb-3 rounded-lg border border-white/5 bg-zinc-950/50 p-4">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
          ...
          <Button
            ...
            disabled={!newCruzid || pendingAction === "add"}
