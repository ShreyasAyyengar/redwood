...
export function FeedFilterControls(props: FeedFilterControlsProps) {
  ...
  const setUrgent = (urgent: boolean) => {
    if (props.kind === "tasks") props.onChange({ ...props.value, urgent });
    else props.onChange({ ...props.value, urgent });
  };
  const setSupervisorNeeded = (supervisorNeeded: boolean) => {
    if (props.kind === "tasks") props.onChange({ ...props.value, supervisorNeeded });
    else props.onChange({ ...props.value, supervisorNeeded });
  };
              ...
              <div className="grid gap-5 md:grid-cols-[1.25fr_1fr]">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="grid gap-2">
                    <CheckboxOption checked={Boolean(props.value.urgent)} label="Urgent" onChange={setUrgent} />
                    <CheckboxOption checked={Boolean(props.value.supervisorNeeded)} label="Needs supervisor" onChange={setSupervisorNeeded} />
                    {props.kind === "tasks" ? (
                      <CheckboxOption
