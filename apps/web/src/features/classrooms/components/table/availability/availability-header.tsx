...
export default function AvailabilityHeader({ column }: { column: Column<ClassroomSummary> }) {
  return (
    <div>
      <div
      ...
      >
        ...
        {column.getIsSorted() ? <ClockCheck className="ml-2 size-6 text-green-500" /> : <SlidersHorizontal className="ml-2 size-6 rotate-90" />}
      </div>
    </div>
  );
}
