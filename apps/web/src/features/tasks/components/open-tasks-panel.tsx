...
import { isOpenTask, sortOpenTasks } from "../model/task-state";
import { TaskCard, TaskCardSkeleton } from "./cards/task-card";
import { ClassroomTaskHistoryDialog } from "./dialogs/classroom-task-history-dialog";
import { TaskDialog } from "./dialogs/task-dialog";

export default function OpenTasks({ room }: { room: Doc<"classrooms"> | undefined }) {
  const tasks = useQuery(api.core.tasks.service.getClassroomTasks, room ? { classroomId: room._id } : "skip");

  if (!room || !tasks) return <OpenTasksSkeleton />;

  const openTasks = sortOpenTasks(tasks.filter((task) => isOpenTask(task)));
