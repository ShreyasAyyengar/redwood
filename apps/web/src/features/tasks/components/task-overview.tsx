...
import { isOpenTask, isOverdueTask, isScheduledTask } from "../model/task-state";
import { TaskListDialog } from "./dialogs/task-list-dialog";

function maybeWrapWithDialog(children: React.ReactNode, filteredTasks: Doc<"tasks">[], title: string) {
...
export default function TaskOverview() {
  const { results: tasks, status, loadMore } = usePaginatedQuery(api.core.tasks.service.getTasks, { view: "OPEN" }, { initialNumItems: 100 });
  useEffect(() => {
  ...
  const scheduled = tasks.filter((task) => isScheduledTask(task));
    ...
    <div>
      <div className="flex items-center">
        ...
        <div className="ml-2 flex flex-col">
          ...
          <span className="text-neutral-400 text-sm">{isFetching ? "Loading..." : `${tasks.length} unfinished tasks`}</span>
        </div>
      </div>
